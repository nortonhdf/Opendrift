...
def lobo_evaluate(X, Y, blocks) -> dict:
    """Leave-one-block-out MAE for the learned model."""
    rows = []
    for held in sorted(set(blocks.tolist())):
        te = blocks == held
        models = fit_model(X[~te], Y[~te])
        rows.append(mae_km(Y[te], predict(models, X[te])))
...
def main() -> None:
    ...
    print(f"Treino surrogate HGB — {len(X)} amostras, "
    ...
    model_m = lobo_evaluate(X, Y, blocks)
    ...
    print(f"[OK] modelo -> {MODEL_OUT.name} | metadados -> {META_OUT.name}")
